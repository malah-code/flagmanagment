      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage user access and assign global or project-level roles.
          </p>
        </div>
        <button
                      ...
                      <td className="px-6 py-4 text-slate-500 text-sm">
                        {user.role === 'Global Administrator' ? (
                          ...
                          user.projects
                            .map(
                              (pid: string) =>
                                projectsData.find((p: any) => p.id === pid)?.name || pid,
                            )
                            .join(', ')
                        ) : (
                          'No Projects'
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto p-2 border border-slate-200 rounded-lg bg-slate-50">
                    ...
                    {projectsData.length === 0 && (
                      <span className="text-xs text-slate-500">No projects found.</span>
                    )}
                  </div>
                </div>
              )}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setEditingUser({
                                ...editingUser,
                                projects: [...editingUser.projects, p.id],
                              });
                            } else {
                              setEditingUser({
                                ...editingUser,
                                projects: editingUser.projects.filter((id) => id !== p.id),
                              });
                            }
                          }}
                        />
