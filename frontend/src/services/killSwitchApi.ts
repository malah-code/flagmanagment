...
export const killSwitchApi = {
  list: async (envId: string, flagId: string): Promise<KillSwitchRule[]> => {
    return await apiClient.get<KillSwitchRule[]>(`/environments/${envId}/flags/${flagId}/kill-switches`);
  },

  create: async (envId: string, flagId: string, alertIdentifier: string): Promise<KillSwitchRule> => {
    return await apiClient.post<KillSwitchRule>(`/environments/${envId}/flags/${flagId}/kill-switches`, {
